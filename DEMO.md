...
- **harness 3086**：dsh web 实例，挂 spec 插件 + 阶段 preset，跑 agent。
...
脚本会：复制插件到 `harness/packages/delivery/` → 改 tsconfig → 注册 workspace → pnpm install → 构建（生成 typert 产物）→ 复制 preset 到 `$DSH_HOME/.agent-presets/`。
...
- **规格**：specStore 里的 `p1/requirements`（需求规格）、`p1/implementation`（实现规格）
- **文件**：`examples/project-delivery/docs/design.md`（设计）、`docs/test-report.md`（测试报告）
- **代码**：编码阶段 agent 真实写的微服务代码（`examples/project-delivery/services/`）

## 收尾：回滚 harness

学习验证完，恢复 harness 零改动：
...
脚本会 git restore + clean 所有临时改动，删除 `$DSH_HOME/.agent-presets`，恢复到 harness 完全干净。
...
| 启动 3086 报 `Cannot find @deepseek-ai/dsh-spec` | web-app 依赖没加 | 重跑 `harness-setup.ps1` 第 4 步 |
| 构建报 TS6059 | delivery tsconfig references 不完整 | 重跑 `harness-setup.ps1`（已含完整 references） |
| 页面显示「离线 mock」而非「实时数据」 | 3086 后端没连上 | 检查 3086 是否在跑、specStore 是否可调 |

## 关键设计说明（想改代码前读）

- **两套 wire 协议**：`/api/session.*`（点号，payload 直接业务参数）vs `/api/specStore/*`（斜杠，payload `{args:{request}}`）。详见 `orchestrator/README.md` 和 `PROJECT.md`。
- **LangGraph interrupt 会重放节点**：`graph.py` 的节点都是幂等的（session_id 存 State）。
- **question 走 `/api/events.mux`**（WebSocket，重放 pending），回答走 `/api/respond`。
